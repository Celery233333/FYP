...
class Measure:
    ...
    def parse_harmony(self, harmony):
        sequence = ""
        root = False
        kind = False
        degree = ""
        ...
        for elem in harmony:
            ...
            if elem.tag == 'kind':
                kind = elem.attrib.get("text")
                if elem.attrib.get("parentheses-degrees") == "yes":
            ...
            if elem.tag == "degree":
                ...
                parentheses = True
                for e in elem:
                    if e.tag == "degree-type":
                        degree = e.text + degree
                        #print(e.text)
                    if e.tag == "degree-value":
                        degree = degree + e.text
                print(degree)

            if kind == False or kind == None:
                sequence = root
            else:
                if degree != "":
                    sequence = str(root)+str(kind)+"("+degree+")"
                else:
                    sequence = str(root)+str(kind)
        #print(sequence)
        return sequence
