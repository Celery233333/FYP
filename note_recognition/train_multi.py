...
# python train_multi.py -voc_p D:\Celery\1\Y4\FYP\sample\polyphonic-omr\experiment_code\vocab\rnn_pitch.txt -voc_r D:\Celery\1\Y4\FYP\sample\polyphonic-omr\experiment_code\vocab\rnn_rhythm.txt -corpus D:\Celery\1\Y4\FYP\corpus1 -l D:\Celery\1\Y4\FYP\sample\polyphonic-omr\experiment_code\models\rnndecoder.pt
...
for epoch in range(max_epochs):
    ...
    for batch_num, batch in enumerate(dataloader_train):
        ...
        loss = pitch_loss(pitch_outs, pitch_targets, out_lengths.clone().detach(), target_lengths)
        ...
        if (batch_num) % 500 == 0:
            ...
            if batch_num == 0:
                ...
                print ('Training loss value at batch %d: %f' % ((batch_num),train_loss),file=f)
                f.close()
                print ('Training loss value at batch %d: %f' % ((batch_num),train_loss))
            else:
                f = open("train_loss.txt","a")
                print ('Training loss value at batch %d: %f' % ((batch_num),train_loss/500),file=f)
                f.close()
    ...
    for batch_num, batch in enumerate(dataloader_valid):
        ...
        with torch.no_grad():
            ...
            greedy_preds_note = utils.greedy_decode(pitch_outs, out_lengths[0])
    ...
    f = open("valid.txt","a")
    print('Validation loss value at epoch %d: %f' % ((epoch+1),valid_loss/len(dataloader_valid)),file=f)
    f.close()
    ...
    # f = open("SER_MSE.txt","a")
    # print('PITCH - Greedy SER at epoch %d: %f' % ((epoch+1), greedy_val_ed_note/greedy_val_len),file=f)
    # print('PITCH - Greedy sequence error rate at epoch %d: %f' % ((epoch+1), (greedy_num_samples-greedy_num_correct_note)/greedy_num_samples),file=f)
    # f.close()
